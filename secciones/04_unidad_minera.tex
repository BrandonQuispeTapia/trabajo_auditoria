...
% --- FIGURA 1: Esquema de ubicación (dibujada en TikZ) ---------------
% Es un CROQUIS SIN ESCALA, no un mapa cartográfico. Representa solo lo que
% las fuentes permiten afirmar: la adyacencia de los dos municipios, las
% tres cuencas y la posición relativa del depósito de relaves al norte de la
% planta. Si el docente exige mapa cartográfico, añade en el Anexo A una
% captura de OpenStreetMap centrada en 15.234852, -91.689418 (la atribución
% «© OpenStreetMap contributors», licencia ODbL, es obligatoria).
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      muni/.style={draw,rounded corners=2pt,minimum width=5.0cm,minimum height=2.3cm,
                   fill=grisclaro,align=center,font=\small,inner sep=5pt},
      infra/.style={draw,circle,inner sep=1.7pt,fill=black},
      rio/.style={grismedio,thick,decorate,
                  decoration={snake,amplitude=.4mm,segment length=3mm}},
      etq/.style={font=\scriptsize,text=grismedio}
    ]
    % --- Cabecera de datos ---
    \node[align=center,font=\scriptsize,anchor=north] at (2.9,4.85)
      {Departamento de \textbf{San Marcos}, altiplano occidental de Guatemala\\
       15\textdegree14\textquotesingle05\textquotedbl{} N,
       91\textdegree41\textquotesingle22\textquotedbl{} O
       \;$\vert$\; 1.800--2.300 \msnm
       \;$\vert$\; $\approx$300 km de la Ciudad de Guatemala};

    % --- Infraestructura, por encima de los municipios ---
    \node[infra] (tsf) at (0,3.25) {};
    \node[font=\scriptsize,anchor=west] at (0.2,3.25) {Depósito de relaves};
    \node[infra] (planta) at (0,2.35) {};
    \node[font=\scriptsize,anchor=west] at (0.2,2.35) {Planta de proceso};
    \draw[-{Stealth[length=2mm]},thin] (planta) -- node[etq,left=1pt] {N} (tsf);

    % --- Municipios ---
    \node[muni] (smi) at (0,0)
      {\footnotesize\textbf{SAN MIGUEL IXTAHUACÁN}\\[3pt]
       \scriptsize Pueblo maya-mam\\[2pt]
       \scriptsize $\approx$87 \% de la propiedad\\
       \scriptsize y del cuerpo mineral};
    \node[muni] (sip) at (5.8,0)
      {\footnotesize\textbf{SIPACAPA}\\[3pt]
       \scriptsize Pueblo maya-sipakapense};

    % --- Límite municipal ---
    \draw[dashed,thick] (2.9,-1.5) -- (2.9,1.5);
    \node[etq,anchor=south] at (2.9,1.6) {límite municipal};

    % --- Ríos ---
    \draw[rio] (-2.6,-2.1) -- (1.2,-2.1);
    \node[etq,anchor=west] at (1.3,-2.1) {río Tzalá};
    \draw[rio] (-2.6,-2.7) -- (2.7,-2.7);
    \node[etq,anchor=west] at (2.8,-2.7) {río Quivichil};
    \draw[rio] (-2.6,-3.3) -- (5.4,-3.3);
    \node[etq,anchor=west] at (5.5,-3.3) {río Cuilco};
    \node[etq,anchor=north west,align=left] at (-2.6,-3.7)
      {Cuencas que delimitan el área de la licencia};
  \end{tikzpicture}
  \caption{Esquema de ubicación de la Mina Marlin y de su entorno hidrográfico}
  \label{fig:mapa-ubicacion}
  \par\vspace{2pt}
  \raggedright\footnotesize
  Nota. Croquis esquemático \textbf{sin escala}; no sustituye a un mapa
  cartográfico. Elaboración propia a partir de \textcite{sec2011ni43101} y
...
% --- FIGURA 2: Diagrama de flujo del proceso (dibujada en TikZ) ------
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      node distance=6mm and 9mm,
      etapa/.style={draw,rounded corners=2pt,minimum width=3.5cm,minimum height=1.0cm,
                    align=center,font=\small,fill=grisclaro},
      critica/.style={etapa,fill=white,very thick},
      prod/.style={draw,rounded corners=6pt,minimum width=3.5cm,minimum height=0.9cm,
                   align=center,font=\small\bfseries},
      fl/.style={-{Stealth[length=2mm]},thick}
    ]
    \node[etapa] (tri) {Trituración};
    \node[etapa,below=of tri] (mol) {Molienda SAG y de bolas\\[-2pt]{\scriptsize 85 \% pasante malla 200}};
    \node[critica,below=of mol] (cil) {Lixiviación con cianuro\\[-2pt]{\scriptsize tipo CIL}};
    \node[etapa,below=of cil] (dec) {Decantación\\[-2pt]{\scriptsize en contracorriente}};
    \node[etapa,below=of dec] (mc)  {Precipitación Merrill-Crowe\\[-2pt]{\scriptsize con zinc}};
    \node[prod,below=of mc] (dore) {Fundición a doré (Au--Ag)};
    ...
    % Rama de relaves
    \node[critica,right=22mm of dec] (inco)
      {Destrucción de cianuro\\[-2pt]{\scriptsize proceso INCO}};
    \node[etapa,below=of inco] (tsf)
      {Depósito de relaves\\[-2pt]{\scriptsize desde 2013, filtrados}};
    \draw[fl] (dec) -- node[above,font=\scriptsize] {sólidos} (inco);
    \draw[fl] (inco) -- (tsf);

    % Leyenda
    \node[font=\scriptsize,align=left,anchor=north west]
      at ($(tsf.south west)+(-0.2,-0.5)$)
      {Los recuadros de borde grueso señalan las dos etapas de mayor\\
       significancia ambiental: el uso de cianuro y su destrucción\\
       previa al depósito de relaves.};
  \end{tikzpicture}
...
\begin{table}[H]
  ...
  \par\vspace{3pt}
  \raggedright\footnotesize
  Nota. Elaboración propia a partir de \textcite{sec2011ni43101}.
\end{table}
...
Conviene añadir una advertencia sobre las fuentes. En publicaciones comerciales y
en anuncios de venta de equipos circulan cifras de capacidad sensiblemente más
altas que las del informe técnico, en algunos casos con promedios superiores a
las 6.000 \tdia. Este informe no las incorpora porque no proceden de documentos
sometidos a un estándar de reporte y su jerarquía probatoria es inferior a la del
informe NI 43-101. Se deja constancia de su existencia para que quien las
encuentre sepa por qué no se recogen aquí.
